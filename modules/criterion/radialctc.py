...
class CTCHelper(object):
    ...
    def decode_batch_path(start, paths, label_list):
        ...
        for t in range(tlgt - 1, 0, -1):
            try:
                ...
                raise ValueError(f"CTC path decoding failed at t={t}, batch={batch}")
    ...
    def ctc_forward(self, logits, label, blank=0, operation="sum"):
        append_label = [
            label[i // 2] if i % 2 == 1 else blank for i in range(len(label) * 2 + 1)
        ]
        len_lgt = len(logits)
        ...
        for t in range(1, len_lgt):
            for s in range(0, len_label):
                ...
                if operation == "sum":
                    dp[t, s] = (
                        self.log_sum_exp([la1, la2, la3]) + logits[t, append_label[s]]
                    )
                else:
    ...
    def batch_ctc_forward(
        self, logits, logits_lgt, label, label_lgt, blank=0, operation="sum"
    ):
        batch_size = logits.shape[1]

        append_label_lgt = label_lgt * 2 + 1
        batch_append_label = np.zeros(
            (label.shape[0], append_label_lgt.max().item()), dtype=int
        )
        ...
        for t in range(1, temporal_lgt):
            for s in range(0, max_append_label_lgt):
                ...
                if s > 1:
                    la3 = dp[:, t - 1, s - 2] * (
                        batch_append_label[:, s] != batch_append_label[:, s - 2]
                    ) + np.ones((batch_size,)) * neginf * (
                        batch_append_label[:, s] == batch_append_label[:, s - 2]
                    )
                else:
                ...
                if operation == "sum":
                    ...
                    dp[:, t, s] = (
                        np.maximum(np.maximum(la1, la2), la3)
                        + logits[t, np.arange(batch_size), batch_append_label[:, s]]
                    )
    ...
    def generate_splits(decoded_path):
        ...
        if st_idx < len(decoded_path):
            splits.append([decoded_path[st_idx], st_idx, ed_idx])
        assert sum([item[2] - item[1] for item in splits]) == len(
            decoded_path
        ), f"{max_path}, {splits}"
        return splits
    ...
    def decode_max_path(self, log_probs, logits_lgt, labels, label_lgt):
        ...
        for sample_idx in range(batch_size):
            sample_probs = self.torch2np(
                log_probs[: logits_lgt[sample_idx], sample_idx]
            )
            sample_labels = labels[sample_idx].tolist()[: label_lgt[sample_idx]]
            dp_max_mat, label_list, lgt, lgt_label, paths = self.ctc_forward(
                sample_probs, sample_labels, blank=0, operation="max"
            )
            start = (
                lgt_label
                - 1
                - np.argmax(
                    [
                        dp_max_mat[lgt - 1, lgt_label - 1],
                        dp_max_mat[lgt - 1, lgt_label - 2],
                    ]
                )
            )
    ...
    def decode_batch_max_path(self, log_probs, logits_lgt, labels, label_lgt):
        ...
        batch_size = log_probs.shape[1]
        splits_list = []
        dp_max_mat, label_list, lgt, lgt_label, paths = self.batch_ctc_forward(
            self.torch2np(log_probs),
            logits_lgt,
            self.torch2np(labels),
            label_lgt,
            blank=0,
            operation="max",
        )
        start_idx = (
            ...
            - np.argmax(
                np.vstack(
                    [
                        dp_max_mat[np.arange(batch_size), logits_lgt - 1][
                            np.arange(batch_size), lgt_label - 1
                        ],
                        dp_max_mat[np.arange(batch_size), logits_lgt - 1][
                            np.arange(batch_size), lgt_label - 2
                        ],
                    ]
                ),
                axis=0,
    ...
    def keyframe_cal(logits, info, lambda_func, batch_multiplier=1):
        ...
        for idx, splits in enumerate(split_list):
            ind_list += [
                (logits[item[1] : item[2], idx, item[0]].argmax() + item[1])
                * batch_multiplier
                + idx
                for item in [*filter(lambda_func, splits)]
            ]
