...
class BiLSTMLayer(nn.Module):
    ...
    def forward(self, src_feats, src_lens, hidden=None):
        ...
        # Ensure contiguous tensors and proper device/dtype for packing
        if not src_feats.is_contiguous():
            src_feats = src_feats.contiguous()
        # pack_padded_sequence expects lengths on CPU as int64
        if src_lens.device.type != "cpu":
        ...
        rnn_outputs, _ = nn.utils.rnn.pad_packed_sequence(
            packed_outputs, total_length=src_feats.size(0)
        )
